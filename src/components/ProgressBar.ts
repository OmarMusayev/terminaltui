...
export function renderProgressBar(label: string, value: number, ctx: RenderContext, options?: { max?: number; showPercent?: boolean }): string[] {
  ...
  const dims = computeBoxDimensions(ctx.width, COMPONENT_DEFAULTS.progressBar);
  ...
  const maxLabelWidth = Math.max(4, Math.floor(dims.content * 0.4));
  const labelWidth = Math.min(stringWidth(label) + 2, maxLabelWidth);
  const barWidth = Math.max(5, dims.content - labelWidth - percentWidth - 2);
