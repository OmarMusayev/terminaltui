...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Wrapped in test() so an app crash here fails a test instead of
    // rejecting main() and aborting the run.
    await test("resize: return home before resize tests", async () => {
      await emu!.goHome();
      await sleep(500);
    });
